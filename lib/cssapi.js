...
const getCSSApi = (file, namespace, sort = false) => {
  ...
  const found = styles.toString().match(new RegExp(`--${namespace}(-*\\w*)*(,(\\s|[^;}!])*)*`, 'g'));

  if (!found) {
    throw new Error(`Style properties not found in ${file}.`);
  }

  const propsAndValues = found.map(item => {
    const [prop, value] = item.split(/,(.+)/); // split by first match of ','
    return [prop, (value || '').trim().replace(/\)$/, '')]; // replace the last ')' by an empty string
  });

  // remove duplicates
  const vars = propsAndValues.map(item => item[0]);
  const results = propsAndValues.filter((prop, index) => vars.indexOf(prop[0]) === index);
...
const mergeResult = (cssProps, cssDocs) => {
  ...
  vars.forEach((prop, index) => {
    ...
    if (found) {
      ...
      result.push([prop.trim(), '', cssProps[index][1].trim()]);
    }
  });

  cssDocs.forEach(prop => {
    if (vars.indexOf(prop[0].trim().replace(/(?!-)\W/g, '')) === -1) {
      removed.push(prop[0]);
    }
  });
 ...
 * @param  {Boolean} sortResult     Sets whether the resulting markdown table should be alphabetically sorted or not. Defaults to false.
 */
const updateCSSDocs = (component, stylesFile, docsFile, sortResult) => {
  getCSSApiFromMarkdownInFile(component, docsFile)
    .then(result => mergeResult(getCSSApi(stylesFile, component, sortResult), result))
