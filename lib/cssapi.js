...
const getCSSPropertiesArray = (arr) => {
  const cssPropertiesAndValues = arr.map((match) => {
    const [property, value] = match.split(/,(.+)/); // split by first match of ','
    return [property, (value || '').trim().replace(/\)$/, '')];
  });

  // remove duplicates
  const getCssProperty = pair => pair[0];
  const cssProperties = cssPropertiesAndValues.map(getCssProperty);
  const results = cssPropertiesAndValues.filter((prop, index) => cssProperties.indexOf(prop[0]) === index);
  return results;
};
