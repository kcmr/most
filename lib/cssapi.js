 ...
 * @param  {String} file      File where the CSS properties will be found. Optional. Default <component>.html
 * @return {Array}            List of CSS properties sorted alphabetically.
 */

// TODO: replace component by namespace and allow file param to be a path
const getCSSApi = (component, file = `${component}.html`) => {
  ...
  const found = styles.toString().match(new RegExp(`--${component}(-*\\w*)*`, 'g'));

  if (!found) {
    // TODO: the second part of the message should not be there (CLI responsibility).
    throw new Error(`Style properties not found in ${file}. Did you forget --file option?`);
  }
...
const updateCSSDocs = (component, stylesFile, docsFile) => {
  getCSSApiFromMarkdownInFile(component, docsFile)
    .then(result => mergeResult(getCSSApi(component, stylesFile), result))
